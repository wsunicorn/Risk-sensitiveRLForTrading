...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const anchor = document.querySelector('a[href="#workflow"]');
  if (anchor) {
...
async function checkStatus() {
  ...
  try {
    ...
    if (data.packages_available) {
      ...
      statusText.textContent = data.models_loaded
        ? "Đã tải mô hình ✓"
        : "Hệ thống sẵn sàng";
    } else {
...
async function getPrediction(model) {
  ...
  document.getElementById("rt-loading-text").textContent =
    `Đang lấy tín hiệu ${model.toUpperCase()} cho ${symbol}...`;
...
function displayPrediction(model, data) {
  ...
  document.getElementById(`${prefix}-timestamp`).textContent =
    `Cập nhật: ${new Date(data.timestamp).toLocaleString()}`;
  ...
  // Action
  document.getElementById(`${prefix}-action`).textContent =
    `Hành động: ${modelData.action.toFixed(3)}`;

  // Strength
  const strength = modelData.strength;
  document.getElementById(`${prefix}-strength`).style.width =
    `${strength * 100}%`;
  document.getElementById(`${prefix}-strength-text`).textContent =
...
function generateAnalysis(model, signal, action, strength, price) {
  ...
  if (model === "ppo") {
    ...
    if (signal === "BUY") {
      ...
      analysis +=
        "Mô hình này tập trung tối đa hóa lợi nhuận mà không có ràng buộc rủi ro rõ ràng.";
    } else if (signal === "SELL") {
    ...
    if (signal === "BUY") {
      ...
      analysis +=
        "Mô hình ưu tiên bảo vệ danh mục trước các kịch bản bất lợi và rủi ro sụt giảm.";
    } else {
      analysis += "CVaR-PPO gợi ý giữ nguyên vị thế hiện tại. ";
      analysis +=
        "Phân tích có xét rủi ro chưa cho thấy cần tái cân bằng danh mục.";
    }
    analysis += "</p>";
  }
...
function showComparison() {
  ...
  if (ppoSignal === cvarSignal) {
    ...
    if (
      ppoSignal === "BUY" &&
      (cvarSignal === "HOLD" || cvarSignal === "SELL")
    ) {
      comparison +=
        "<li><strong>Diễn giải:</strong> PPO thấy cơ hội lợi nhuận nhưng CVaR-PPO thận trọng hơn do xét đến rủi ro.</li>";
    } else if (
      cvarSignal === "BUY" &&
      (ppoSignal === "HOLD" || ppoSignal === "SELL")
    ) {
      comparison +=
        "<li><strong>Diễn giải:</strong> CVaR-PPO nhận diện cơ hội đã điều chỉnh theo rủi ro mà PPO không ưu tiên.</li>";
    }
  }
  ...
  if (Math.abs(ppoStrength - cvarStrength) > 20) {
    comparison +=
      "<li><strong>Ghi chú:</strong> Chênh lệch lớn về độ mạnh tín hiệu cho thấy hai mô hình diễn giải thị trường khác nhau.</li>";
  }
...
function displayBacktestResults(results, dataInfo) {
  ...
  document.getElementById("bt-period").textContent =
    `${dataInfo.start_date} đến ${dataInfo.end_date}`;
...
function createPortfolioChart(results) {
  ...
  Plotly.newPlot("bt-chart", [trace1, trace2, trace3], layout, {
    responsive: true,
  });
}
...
function displayComparison(comparison) {
  ...
  // PPO
  document.getElementById("cmp-ppo-return").textContent =
    `${(results.PPO.total_return * 100).toFixed(2)}%`;
  document.getElementById("cmp-ppo-sharpe").textContent =
    results.PPO.sharpe_ratio.toFixed(4);
  document.getElementById("cmp-ppo-drawdown").textContent =
    `${(results.PPO.max_drawdown * 100).toFixed(2)}%`;
  document.getElementById("cmp-ppo-winrate").textContent =
    `${(results.PPO.win_rate * 100).toFixed(2)}%`;
  document.getElementById("cmp-ppo-volatility").textContent =
    `${(results.PPO.volatility * 100).toFixed(2)}%`;

  // CVaR-PPO
  document.getElementById("cmp-cvar-return").textContent =
    `${(results["CVaR-PPO"].total_return * 100).toFixed(2)}%`;
  document.getElementById("cmp-cvar-sharpe").textContent =
    results["CVaR-PPO"].sharpe_ratio.toFixed(4);
  document.getElementById("cmp-cvar-drawdown").textContent =
    `${(results["CVaR-PPO"].max_drawdown * 100).toFixed(2)}%`;
  document.getElementById("cmp-cvar-winrate").textContent =
    `${(results["CVaR-PPO"].win_rate * 100).toFixed(2)}%`;
  document.getElementById("cmp-cvar-volatility").textContent =
    `${(results["CVaR-PPO"].volatility * 100).toFixed(2)}%`;

  // Buy & Hold
  document.getElementById("cmp-bh-return").textContent =
    `${(results["Buy & Hold"].total_return * 100).toFixed(2)}%`;
  document.getElementById("cmp-bh-sharpe").textContent =
    results["Buy & Hold"].sharpe_ratio.toFixed(4);
  document.getElementById("cmp-bh-drawdown").textContent =
    `${(results["Buy & Hold"].max_drawdown * 100).toFixed(2)}%`;
  document.getElementById("cmp-bh-winrate").textContent =
    `${(results["Buy & Hold"].win_rate * 100).toFixed(2)}%`;
  document.getElementById("cmp-bh-volatility").textContent =
    `${(results["Buy & Hold"].volatility * 100).toFixed(2)}%`;
...
function createRadarChart(results) {
  ...
  const normalizeVolatility = (v) =>
    Math.max(0, Math.min(100, (1 - v / 0.2) * 100)); // Invert volatility
  ...
  new Chart(ctx, {
    ...
    data: {
      labels: [
        "Lợi nhuận",
        "Tỷ lệ Sharpe",
        "Kiểm soát rủi ro (sụt giảm)",
        "Tỷ lệ thắng",
        "Ổn định (biến động thấp)",
      ],
      datasets: [
        {
          label: "PPO",
